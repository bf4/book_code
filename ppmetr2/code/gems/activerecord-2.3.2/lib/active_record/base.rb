          ...
          if match.finder?
            ...
            self.class_eval %{
              def self.#{method_id}(*args)
                ...
                #{'result || raise(RecordNotFound, "Couldn\'t find #{name} with
                  #{attributes.to_a.collect {|pair|
                    "#{pair.first} = #{pair.second}"}.join(\', \')}")' if bang}
              end
            }, __FILE__, __LINE__
            ...
            self.class_eval %{
              def self.#{method_id}(*args)
                ...
                if record.nil?
                  record = self.new { |r|
                    r.send(:attributes=, attributes, guard_protected_attributes)
                  }
