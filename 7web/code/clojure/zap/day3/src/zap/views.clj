...
;
(deftemplate base-page "templates/projects.html" ;;(1)
  [title & body]
  [:title] (content title)
...
(defsnippet admin-bar
  ...
  [:a (but first-child)] nil ;;(2)
  [:a] (clone-for [[url title] links] ;;(3)
         (do->
          (set-attr :href url)
          (content title))))
...
(defn create-project [params]
  (let [params (select-keys params [:name])]
    (if-not (valid/valid-project? params)
      {:status 400
       :body (json/write-str {:error "Project data invalid."})}
      (do
        (models/create-project params)
        {:status 200
         :body ""}))))
