...
class LSystem
  def initialize(rules={})
    @rules = {}
    rules.each do |match, replacement|
      @rules[match.to_s] = replacement.to_s.split(//)
    end
  end

  def run(iterations, axiom)
    result = axiom.to_s.split(//)
    iterations.times do
      result.map!{|current| @rules[current] || current }.flatten!
    end
    return result.join
  end
end