...
class CuteUnitTile < CuteTile
  def destination(col, row)
    OSX::NSRect.new(col * @width, row * height + CUTE_TILE_OFFSET_Y, @width, @height)
  end
end
...
class CocoaPlayer < BasePlayer
  ...
  def create_menu
    ...
    quit_item = app_menu.addItemWithTitle_action_keyEquivalent("Quit DinoWars", :terminate_, "q")
...
class DinoCocoaPlayer < CocoaPlayer
  TERRAIN = {
    'Grass'  => CuteTile.new('PlanetCute/Grass Block.png'),
    'Water'  => CuteTile.new('PlanetCute/Water Block.png'),
    'Forest' => MergedTile.new(CuteTile.new('PlanetCute/Grass Block.png'), CuteTile.new('PlanetCute/Tree Ugly.png')),
    'Plains' => CuteTile.new('PlanetCute/Stone Block.png'),
  }
  UNITS = {
    'Captain' => CuteUnitTile.new('PlanetCute/Character Boy.png'),
    'Doctor'  => CuteUnitTile.new('PlanetCute/Character Pink Girl.png'),
    'Soldier' => CuteUnitTile.new('PlanetCute/Character Horn Girl.png'),
    'VRaptor' => CuteUnitTile.new('PlanetCute/Enemy Bug.png'),
  }
  W = TERRAIN.values.first.width
  H = TERRAIN.values.first.height
  HIGHLIGHTS = {
    'Unit'  => CuteColorTile.new(W, H, 0.8, 0.4, 0.0, 0.5),
    'Shoot' => CuteColorTile.new(W, H, 1.0, 0.0, 0.0, 0.5),
    'Move'  => CuteColorTile.new(W, H, 0.1, 0.1, 1.0, 0.5),
  }
  COLS = 10
  ROWS = 6
