...
class Action
  ...
  def self.generate(unit, game)
    map = game.map
    near_positions = map.near_positions(range(unit), unit.x, unit.y)
    target_positions = near_positions.find_all{|x, y| target?(unit, map.units[x, y]) }
    return target_positions.collect{|x, y| self.new(unit, game, x, y) }
  end
