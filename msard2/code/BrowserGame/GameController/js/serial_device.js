...
SerialDevice.prototype.onReceive = function(receiveInfo) {
  ...
  while ((index = this.readBuffer.indexOf('\n')) >= 0) {
    var line = this.readBuffer.substr(0, index + 1);
    this.onReadLine.dispatch(line);
    this.readBuffer = this.readBuffer.substr(index + 1);
  }
};
...
SerialDevice.prototype.send = function(msg) {
  if (this.connectionId < 0) {
    throw 'Invalid connection';
  }
  chrome.serial.send(this.connectionId, this.stringToArrayBuffer(msg), function() {});
};
