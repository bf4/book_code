...
class TweetDetailViewController: UIViewController, TwitterAPIRequestDelegate {
  ...
  // note to self: remember to make button "custom" in storyboard, or it will ignore our image
...
// comment this out, or die with 12: fatal error: use of unimplemented initializer 'init(coder:)' for class 'PragmaticTweets.TweetDetailViewController'
      ...
      if let dataValue = data {
        ...
        if let tweetDict = jsonObject as? Dictionary<String, AnyObject> {
          dispatch_async(dispatch_get_main_queue(),
            {() -> Void in
              let userDict = tweetDict["user"] as NSDictionary // THIS IS HORRIBLE. THANK YOU, APPLE.
