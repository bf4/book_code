...
class RootViewController: UITableViewController, TwitterAPIRequestDelegate, UISplitViewControllerDelegate {
  ...
  override func viewDidLoad() {
    ...
    // TODO: switch to "self.splitViewController?.delegate = self" ?
  ...
  func reloadTweets() {
    ...
    // the twitter API in this string does NOT exist. we are deliberately sending
    // a bogus request so we can learn to debug the response handling.
    // (in other chapters, the API is "https://api.twitter.com/1.1/statuses/home_timeline.json")
    let twitterAPIURL = NSURL.URLWithString(
      "https://api.twitter.com/1.1/statuses/foo_bar.json")
