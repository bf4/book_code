...
public class RootViewController: UITableViewController, TwitterAPIRequestDelegate {
 ...
 @IBAction func handleTweetButtonTapped(sender: AnyObject) {
  ...
  @IBAction func handlePhotoButtonTapped (sender: UIBarButtonItem) {
    var fetchOptions = PHFetchOptions() // 1
    PHPhotoLibrary.requestAuthorization { (authorized: PHAuthorizationStatus) -> Void in
      if authorized == .Authorized { // 2
        fetchOptions.sortDescriptors = [NSSortDescriptor(key: "creationDate", ascending: false)] // 3
        let fetchResult = PHAsset.fetchAssetsWithMediaType(PHAssetMediaType.Image, options: fetchOptions) // 4
  ...
  */
  ...
  func createTweetForAsset (asset: PHAsset) {
    var requestOptions =  PHImageRequestOptions() // 1
    requestOptions.synchronous = true // 2
    
    PHImageManager.defaultManager().requestImageForAsset(asset,
      targetSize: CGSizeMake(640, 480), // 3
      contentMode: PHImageContentMode.AspectFit, // 4
      options: requestOptions) { (image:UIImage!, info: [NSObject : AnyObject]!) -> Void in
        ...
        let message = NSLocalizedString(
          ...
          comment:"")
        tweetVC.setInitialText(message) // 6
        tweetVC.addImage(image) // 7
